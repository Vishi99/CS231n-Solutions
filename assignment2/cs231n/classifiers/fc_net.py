...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        fc1, c1 = affine_forward(X, self.params['W1'], self.params['b1'])
        relu1, c2 = relu_forward(fc1)
        fc2, c3 = affine_forward(relu1, self.params['W2'], self.params['b2'])
        scores = fc2
        ...
        dx, dw2, db2 = affine_backward(dx, c3)
        dx = relu_backward(dx, c2)
        dx, dw1, db1 = affine_backward(dx, c1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(0, len(layer_dims)-1):
            Wi = "W"+str(i+1)
            bi = "b"+str(i+1)
            self.params[Wi] = np.random.normal(0, weight_scale, (layer_dims[i], layer_dims[i+1]))
            self.params[bi] = np.zeros(layer_dims[i+1])
            if(normalization != None and i < len(layer_dims)-2):
