...
def affine_backward(dout, cache):
    ...
    # pass
    dx_trans = np.dot(dout, np.transpose(w))
    dx = np.reshape(dx_trans, x.shape)
    x_trans = np.reshape(x, (x.shape[0], -1))
...
def batchnorm_backward(dout, cache):
    ...
    dgamma = np.sum(cache['x_c']*dout, axis = 0)

    dx_c = ([cache['gamma']]*N)*dout
    dvar = -0.5*np.power(cache['var'], -1.5)*np.sum(dx_c*(cache['x']-cache['mean']), axis = 0)
    dmean = np.sum(dx_c, axis = 0)*(-1.0/np.sqrt(cache['var']))
    dmean += dvar*(-2.0/N)*np.sum((cache['x']-cache['mean']), axis = 0)

    dx = dx_c*(1/np.sqrt(cache['var']))
    dx += dvar*2*(cache['x']-cache['mean'])/N
    dx += [dmean/N]*N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = dx_c - x_c*np.sum(dx_c*x_c, axis=0)/N - dx_c_sum/N
    dx /= std
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N):
        for j in range(C):
            x_padded[i, j] = np.pad(x[i, j], pad_width=pad)

    for i in range(N):
        for f in range(F):
            for r in range(0, H + 2*pad - HH + 1, stride):
                for c in range(0, W + 2*pad - WW + 1, stride):
                    out[i, f, r//stride, c//stride] = np.sum(x_padded[i, :, r : r+HH, c : c+WW] * w[f]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(N):
        for f in range(F):
            for r in range(0, H + 2*pad - HH + 1, stride):
                for c in range(0, W + 2*pad - WW + 1, stride):
                    dw[f] += x_padded[i, :, r : r+HH, c : c+WW]*dout[i, f, r//stride, c//stride]
                    dx_padded[i, :, r : r+HH, c : c+WW] += w[f]*dout[i, f, r//stride, c//stride]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for ch in range(C):
            for r in range(0, H - pool_height + 1, stride):
                for c in range(0, W - pool_width + 1, stride):
                    out[i, ch, r//stride, c//stride] = np.max(x[i, ch, r: r+pool_height, c : c+pool_width])
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for ch in range(C):
            for r in range(0, H - pool_height + 1, stride):
                for c in range(0, W - pool_width + 1, stride):
                    frame = x[i, ch, r : r + pool_height, c : c + pool_width]
                    ro , co = np.unravel_index(np.argmax(frame), frame.shape)
                    dx[i, ch, r + ro, c + co] += dout[i, ch, r//stride, c//stride]
