...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #pass
            temp = np.tile(X[i], (self.X_train.shape[0], 1))
            dists[i] += np.sum(np.power(self.X_train, 2), axis=1)
    ...
    def compute_distances_no_loops(self, X):
        ...
        sum_train = [np.sum(np.power(self.X_train, 2), axis = 1)]*num_test
        dists += sum_train

        dists -= 2*np.matmul(X, np.transpose(self.X_train))
    ...
    def predict_labels(self, dists, k=1):
        ...
        sorted = np.argsort(dists, axis = 1)
        for i in range(num_test):
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            closest_y = list(self.y_train[sorted[i]])
