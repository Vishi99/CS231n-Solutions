...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        h0, cache_h1 = affine_forward(features, W_proj, b_proj)
        ...
        out_vcb, cache_vcb = temporal_affine_forward(h, W_vocab, b_vocab)

        loss, dx = temporal_softmax_loss(out_vcb, captions_out, mask)

        dx, dW_vocab, db_vocab = temporal_affine_backward(dx, cache_vcb)
        ...
        dfeatures, dW_proj, db_proj = affine_backward(dh0, cache_h1)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                next_h, _ = rnn_step_forward(embed, next_h, Wx, Wh, b)
            if self.cell_type == 'lstm':
                next_h, next_c, _ = lstm_step_forward(embed, next_h, next_c, Wx, Wh, b)

            out_vocab = np.dot(next_h, W_vocab) + b_vocab
            prev_word = np.argmax(out_vocab, axis = 1)
