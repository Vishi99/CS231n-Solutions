...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((T, N, H))
    for i in range(T):
        h_next, cache_i = rnn_step_forward(x[i], h_next, Wx, Wh, b)
        cache[i] = cache_i
        h[i] = h_next
...
def rnn_backward(dh, cache):
    ...
    for i in range(len(cache)-1, -1, -1):
        dx_i, dprev_h, dWxi, dWhi, dbi  = rnn_step_backward(dprev_h+dh[i], cache[i])
        dx[i] += dx_i
        dWx += dWxi
        dWh += dWhi
        db += dbi
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # pass
    N, H = prev_c.shape
    act = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    ai, af, ao, ag = act[:, : H], act[:, H: 2*H], act[:, 2*H: 3*H], act[:, 3*H: 4*H]
    i = sigmoid(ai)
    f = sigmoid(af)
    o = sigmoid(ao)
    g = np.tanh(ag)
    ...
    cache = {'x':x, 'act':act, 'i':i, 'f':f, 'o':o, 'g':g, 'next_c':next_c,
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    di = cache['g']*dnext_c
    dg = cache['i']*dnext_c
    do = np.tanh(cache['next_c'])*dnext_h

    dai = cache['i']*(1 - cache['i'])*di
    daf = cache['f']*(1 - cache['f'])*df
    dao = cache['o']*(1 - cache['o'])*do
    dag = (1 - cache['g']**2)*dg

    dact = np.concatenate((dai, daf, dao, dag), axis = 1)
    dx = np.dot(dact, cache['Wx'].T)
    dWx = np.dot(cache['x'].T, dact)
    dprev_h = np.dot(dact, cache['Wh'].T)
    dWh = np.dot(cache['prev_h'].T, dact)
...
def lstm_backward(dh, cache):
    ...
    for i in range(len(cache)-1, -1, -1):
        dx_i, dprev_h, dprev_c, dWxi, dWhi, dbi  = lstm_step_backward(dprev_h+dh[i], dprev_c,cache[i])
        dx[i] += dx_i
        dWx += dWxi
        dWh += dWhi
        db += dbi
